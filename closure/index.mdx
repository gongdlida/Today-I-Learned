...
클로저
생명주기가 종료된 외부함수의 변수를 참조하는 종속함수

## 클로저 동작원리

정의된 함수가 평가될 때, 함수 객체의 내부슬롯 [[Environment]] 에
해당 함수가 선언된 위치의 상위 렉시컬 환경(Lexical Environment)의 참조가 저장된다.

내부 함수가 외부 함수의 변수를 참조하고 있다면,
비록 외부 함수의 실행 컨텍스트는 종료되더라도
렉시컬 환경은 내부 함수에 의해 참조되고 있으므로
가비지 컬렉터는 외부 함수의 환경 레코드(Environment Record) 는 제거하지 않는다.
...
[ Memory - Lexical Environments ]
Outer LE ← inner 함수가 계속 참조 중이므로 유지
...
내부 함수에 대한 모든 참조를 끊어줘야(undefined이나 null 할당)
